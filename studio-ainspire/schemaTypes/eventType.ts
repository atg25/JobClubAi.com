...
export const eventType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((value, {parent}) =>
          (parent as {type?: string})?.type === 'In-Person' ||
          (parent as {type?: string})?.type === 'Hybrid'
            ? value
              ? true
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom((value, {parent}) =>
          (parent as {type?: string})?.type === 'Virtual' ||
          (parent as {type?: string})?.type === 'Hybrid'
            ? value
              ? true
              : 'Link is required for Virtual or Hybrid events'
            : true,
        ),
    }),
  ],
  preview: {
    ...
    prepare({title, category, datetime, createdAt, updatedAt}) {
      ...
      return {
        ...
        subtitle,
        description: `Last updated: ${updatedStr}`,
      }
    },
  },
})
